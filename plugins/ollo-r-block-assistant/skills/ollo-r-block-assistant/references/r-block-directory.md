...
### Rotate
* **Exact Text:** `Rotate [Dropdown Icon] in the back axis of [Dropdown] at speed [Value] .`
* **Remarks:** Features an icon-based configuration dropdown (showing a clock/gauge symbol), a text dropdown (defaulted to `Left`), a numeric input field (defaulted to `50`), and concludes with a trailing period.
...
* **Remarks:** Features a starting channel dropdown (defaulted to `No.1`), followed by a degree target numeric input field (defaulted to `0°`), an execution speed numeric input field (defaulted to `50`), and concludes with a trailing period.
...
### Follow black line at a speed of
* **Exact Text:** `Follow black line at a speed of [Value] with [Dropdown]`
* **Remarks:** Features an embedded numeric input field (defaulted to `50`) and a trailing dropdown selection menu (defaulted to `WHEEL ROBOT(BACK_AXIS)`).

### End line following
* **Exact Text:** `End line following`
* **Remarks:** Standard standalone inline text block.

### If crossroad shape is
* **Exact Text:** `If crossroad shape is [Dropdown] turn`
* **Remarks:** Features an embedded text dropdown menu selector defaulted to `Left`.

### Turn on the line
* **Exact Text:** `Turn [Dropdown] on the line`
* **Remarks:** Pointed block shape featuring a dropdown menu displaying a line graphic segment selection.
